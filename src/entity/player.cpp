...
Player::Player(glm::vec3 position) {
  ...
  GLfloat vertices[4 * 3] = {
      -TSIZE / 2.0f, -TSIZE / 2.0f, 0.0f, -TSIZE / 2.0f, TSIZE / 2.0f,  0.0f,
      TSIZE / 2.0f,  TSIZE / 2.0f,  0.0f, TSIZE / 2.0f,  -TSIZE / 2.0f, 0.0f};
...
void Player::update() {
  if (Input::key_pressed(SDL_SCANCODE_W)) {
    ...
    //        if (level->raycast_block_id(position, rot, 32.0f) < 2 &&
    //        level->raycast_block_id(pos, rot, 32.0f) < 2) {
    position.x += xa;
    position.z += za;
    //        }
  }
...
void Player::render() {
  ...
  if (light)
    shader->set_uniform_float1("light_on", 1);
  else
    shader->set_uniform_float1("light_on", 0);
  shader->disable();
  Shader::HUD->enable();
  target->render();
  Shader::HUD->disable();
