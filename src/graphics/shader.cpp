...
void Shader::DESTROY() { delete BLOCK; }
...
GLint Shader::get_uniform(const GLchar *name) {
  ...
  if (result == -1)
    std::cerr << "Could not find location for '" << name << "'" << std::endl;
  return result;
}
...
void Shader::enable() { glUseProgram(shader); }
